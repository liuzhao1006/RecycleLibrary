...
public class LzRefreshView extends LinearLayout {
     ...
     * 被刷新的view
     */
    private LzRefreshContentView mContentView;
    ...
    private LzRefreshViewState mState = null;
    /**
    ...
    public LzRefreshView(Context context) {
        this(context,null);
    }

    public LzRefreshView(Context context, @Nullable AttributeSet attrs) {
        this(context, attrs,-1);
    }

    public LzRefreshView(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        setClickable(true);
        setLongClickable(true);
        mContentView = new LzRefreshContentView();
        mHolder = new LzRefreshHolder();
    ...
    public void setOnBottomLoadMoreTime(OnLoadMore bottomListener) {
        mContentView.setOnLoadMore(bottomListener);
    }
    ...
    private void addHeaderView() {
        if (mHeaderView == null) {
            mHeaderView = new LzRefreshViewHeader(getContext());
        }
        dealAddHeaderView();
    }
    ...
    private void attachContentView() {
        mContentView.setContentView(LzRefreshView.this.getChildAt(1));
    ...
    private void addFooterView() {
        if (mFooterView == null) {
            mFooterView = new LzRefreshViewFooter(getContext());
        }
        dealAddFooterView();
    }
...
//        if(mHolder.mOffsetY!=0)return;
            ...
            case MotionEvent.ACTION_UP:
                ...
                if (mHolder.hasHeaderPullDown()) {
                    if (mEnablePullRefresh && !mStopingRefresh && !mPullRefreshing && mHolder.mOffsetY > mHeaderViewHeight) {
                        ...
                        mState = LzRefreshViewState.STATE_REFRESHING;
    ...
    public LzRefreshContentView getContentView() {
        return mContentView;
    }
            ...
            if (mEnablePullRefresh && !mPullRefreshing) {
                if (mHolder.mOffsetY > mHeaderViewHeight) {
                    if (mState != LzRefreshViewState.STATE_READY) {
                        ...
                        mState = LzRefreshViewState.STATE_READY;
                    }
                } else {
                    if (mState != LzRefreshViewState.STATE_NORMAL) {
    ...
    private void updateFooterHeight(int deltaY) {
        if (mEnablePullLoad) {
            if (needAddFooterView()) {
                if (isEmptyViewShowing()) {
                    ...
                    if (mState != LzRefreshViewState.STATE_LOADING) {
                        mFooterCallBack.onStateRefreshing();
                        mState = LzRefreshViewState.STATE_LOADING;
                    }
                }
        ...
        if (needAddFooterView() || enableRecyclerViewPullUp) {
            if (mEnablePullUpWhenLoadCompleted || !mContentView.hasLoadCompleted()) {
               ...
               /* if (!needAddFooterView() && mContentView.getState() != LzRefreshViewState.STATE_COMPLETE && autoLoadMore) {
...
//            headerMovePercent = headerMovePercent > 1 ? 1 : headerMovePercent;
    ...
    public void stopRefresh(boolean success) {
        if (mPullRefreshing == true) {
            ...
            mState = LzRefreshViewState.STATE_COMPLETE;
...
//        mFooterCallBack.onStateRefreshing();
    ...
    public void setXRefreshViewListener(XRefreshViewListener l) {
        ...
        mContentView.setLzRefreshViewListener(l);
    }
